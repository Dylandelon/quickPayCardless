...
public class Sm4UtilsTest {
	...
	private static Logger logger = Logger.getLogger(Sm4UtilsTest.class);
	...
	public void testSm4Cbc() {
		...
		byte[] key = sm4Key.getBytes();
		String encode = null, decode = null;
		
		byte[] srcHex = src.getBytes();
		byte[] result = Sm4Utils.sm4CbcEncrypt(key, srcHex, Constants.PAD_NO);
		try {
		...
		byte[] deresult = Sm4Utils.sm4CbcDecrypt(key, desrcHex, Constants.PAD_NO);
		try {
	...
	public void testSm4Ecb() {
		...
		byte[] result = Sm4Utils.sm4EcbEncrypt(key, src.getBytes(), Constants.PAD_NO);
		try {
			encode = new String(Base64.encodeBase64(result), charset);
		} catch (UnsupportedEncodingException e) {
			logger.error("Fail: ", e);
		...
		byte[] desrcHex = Base64.decodeBase64(encode);
		byte[] deresult = Sm4Utils.sm4EcbDecrypt(key, desrcHex, Constants.PAD_NO);
		try {
			decode = new String(deresult, charset);
		} catch (UnsupportedEncodingException e) {
			logger.error("Fail:", e);
