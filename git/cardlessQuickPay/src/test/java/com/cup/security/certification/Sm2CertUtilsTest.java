...
public class Sm2CertUtilsTest {
	Logger logger = Logger.getLogger(Sm2CertUtilsTest.class);
	private static String path = ClassLoader.getSystemResource("").getPath();

	@Test
	public void testGetPubKeyFPubCert() {
		String filePath;
		filePath = path + "/sm2Pub.cer";
		
		CipherParameters cipherParameters = Sm2CertUtils.getPubKeyFPubCert(filePath);
		logger.info("pubcer:\n" + cipherParameters);
	}
