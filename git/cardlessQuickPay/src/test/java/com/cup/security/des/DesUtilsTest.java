...
public class DesUtilsTest {
	private static Logger logger = Logger.getLogger(DesUtilsTest.class);

	@Test
	public void testDesEdeCbc() {
		String charset = "utf-8";
		String src = "12345asdfg";
		String desedeKey = "123456782234567832345678";

		byte[] key = desedeKey.getBytes();
		String encode = null, decode = null;

		byte[] result = DesUtils.desEdeCbcEncrypt(key, src.getBytes(), Constants.PAD_NO);
		try {
		...
		byte[] deresult = DesUtils.desEdeCbcDecrypt(key, desrcHex, Constants.PAD_NO);
		try {
	...
	public void testDesEdeEcb() {
		...
		// byte[] result = DesUtils.DesEdeEcbEncrypt(key, src.getBytes(), Constants.PAD_NO);
		byte[] result = DesUtils.desEdeEcbEncrypt(key, src.getBytes(), Constants.PAD_PKCS5);
		try {
			encode = new String(Base64.encodeBase64(result), charset);
		} catch (UnsupportedEncodingException e) {
			logger.error("Fail: ", e);
		...
		byte[] desrcHex = Base64.decodeBase64(encode);
		// byte[] deresult = DesUtils.DesEdeEcbDecrypt(key, desrcHex, Constants.PAD_NO);
		byte[] deresult = DesUtils.desEdeEcbDecrypt(key, desrcHex, Constants.PAD_PKCS5);
		try {
			decode = new String(deresult, charset);
		} catch (UnsupportedEncodingException e) {
			logger.error("Fail:", e);
